...
<div id="jadval">
    <table class="table table-hover table-bordered">
        ...
        foreach ($rows as $row)
        {
            ...
            <td>'.$row["name"].'</td>
            <td>'.$row["select"].'</td>';
            if ($row["state"] == 1){echo '<td>اولین اسلایدر</td>';}
            if ($row["state"] == 2){echo '<td>دومین اسلایدر</td>';}
            if ($row["state"] == 3){echo '<td>سومین اسلایدر</td>';}
            if ($row["state"] == 4){echo '<td>چهارمین اسلایدر</td>';}
       echo '<td><a href="#" class="delete-slider stClass" id='.$row["id"].'>حذف <i class="fas fa-minus"></i></a></td>
            </tr>
            ';
            $i++;
        }
